...
<head>
    ...
    <title>中华医学会期刊资源云服务选购平台</title>
</head>

<body class="login-wrap bgImg">
    <div class="login-container">
        <form class="layui-form login-form">
            <p class="cr-333 fz-20 text-c" style="margin-bottom:25px">重置密码</p>
            ...
            <div class="layui-form-item" style="position:relative;height:50px">
                <input class="fl" type="text" name="verfiy" required style="width:195px;position:absolute" value=""
                    lay-verify="required|verfiy" placeholder="请输入验证码" autocomplete="off" class="layui-input">
                <button type="button" class="layui-btn login-btn fr login-btn fr sendCode"
            ...
            <div class="layui-form-item password">
                <input type="password" name="repassword" required value="" lay-verify="required|pass" maxlength="20" placeholder="再次输入密码" lay-reqtext="请再次输入密码"
                    autocomplete="off" class="layui-input">
            </div>
            <button type="button" class="layui-btn login-btn" lay-submit lay-filter="save"
                style="width:100%;height:49px;font-size: 21px">保存</button>
            <div class="text-c fz-18" style="margin-top: 25px">
                <span style="color:#999999;">想起密码？</span><span><a href="${ctx}/login.html"
...
<style>
    .bgImg {
        background: url('${ctx}/static/assets/images/bg.jpg')
    }
</style>
...
<script>
    layui.use(['form', 'jquery','common'], function () {
        ...
        form.on('submit(save)', function (data) {
            ...
            //表单数据formData
            common.ajax('post','${ctx}/login/repassword.html',data.field,function(res){
            	if(res.success){
            		layer.msg('密码重置成功!', { icon: 6, anim: 6, time: 1000,  shade: [0.8, '#393D49'] },function(){
        ...
        $(document).on('click','#smsButtom',function(){
        	...
        	if(phone == null || phone == 'undefined' || phone ==''|| !regexp.test(phone)){
        	 ...
        	 common.ajax('post','${ctx}/login/sendSms.html',{phone:phone,tranType:3},function(res){
             	if(res.success){
             		var count = 60;
