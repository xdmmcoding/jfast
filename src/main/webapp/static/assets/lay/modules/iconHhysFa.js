...
layui.define(['laypage', 'form'], function (exports) {
    ...
    IconHhys.prototype.render = function (options) {
        var opts = options,
            ...
            closeIcon = opts.closeIcon == null ? false : opts.closeIcon,
            // json数据
            data = {},
            // 唯一标识
        ...
        var a = {
            init: function () {
                ...
                }else if(type == "ali-svg"){
                	data = common.getDJson(url);
                }else{
               ...
               a.hideElem().createSelect().createBody().toggleSelect();
                        ...
                        common.loadCss();
                        if (success) {  
                        	if(type == "ali-svg" && closeIcon){
                        		var btn= document.getElementById('ali-close');
                        		btn.onclick = function(){
	                        		var el = $(elem),
	                                    p = el.next().find('.layui-iconpicker-item .layui-icon-ali-ic').find('use'),
	                                    c = '';
	                                if (c.indexOf('#xe') > -1){
	                                    p.html(c);
	                                } else {
	                                	p.html('').attr('xlink:href', '#'+c);
	                                }
            ...
            createSelect: function () {
                if (type.indexOf("fontClass") > -1){
                	...
                	var oriIcon = '<svg class="layui-icon-ali-ic" aria-hidden="true">';
                    // 默认图标
                    if (ORIGINAL_ELEM_VALUE === '') {
                        ORIGINAL_ELEM_VALUE = '';
                    }
                    oriIcon = '<div style="line-height: 38px;font-size: 18px;margin-left:5px;"><svg class="layui-icon-ali-ic" aria-hidden="true"><use xlink:href="#'+ORIGINAL_ELEM_VALUE+'"></use></div>';
                    oriIcon += '</svg>';
                    if(closeIcon)
                    	oriIcon +='<i class="layui-icon ali-close layui-icon-close" style="font-size: 16px;" id="ali-close"></i>';
                    var selectHtml = '<div class="layui-iconpicker layui-unselect layui-form-select" id="' + ICON_BODY + '">' +
                        '<div class="' + TITLE + '" id="' + TITLE_ID + '">' +
                        '<div class="layui-iconpicker-item">' +
                        '<span class="layui-iconpicker-icon layui-unselect">' +
            ...
            createList: function (text) {
                ...
                for (var i = 0; i < l; i++) {
                    ...
                    if(type.indexOf("fontClass") >-1){
                    	...
                    	icon += '<svg style="line-height: 38px;font-size: 18px;" class="layui-icon-ali-ic" aria-hidden="true"><use xlink:href="#'+obj+'"></use></svg>';
                    }else {
            ...
            check: function () {
                if(type.indexOf("fontClass") >-1){
                	...
                	var item = '#' + PICKER_BODY + ' .layui-iconpicker-icon-item';
                    a.event('click', item, function (e) {
                        var el = $(e.currentTarget).find('.layui-icon-ali-ic').find('use'),
                            icon = '';
                        
                        var clsArr = el.attr('xlink:href').split(/[\s\n]/),
                            cls = clsArr,
                            icon = cls;
                        $('#' + TITLE_ID).find('.layui-iconpicker-item .layui-icon-ali-ic').find('use').html('').attr('xlink:href', clsArr.join(' '));
            	...
            	$.ajax({
            		...
            		success: function (ret) {
            			for(var i=0;i<ret.glyphs.length;i++){
            				iconlist.push(ret.css_prefix_text+ret.glyphs[i].font_class);
            			}
            		},
            		error: function (xhr, textstatus, thrown) {
            			layer.msg('fa图标接口有误');
    ...
    IconHhys.prototype.checkIconAli = function (filter, iconName){
        var el = $('*[lay-filter='+ filter +']'),
            p = el.next().find('.layui-iconpicker-item .layui-icon-ali-ic').find('use'),
            c = iconName;
        if (c.indexOf('#xe') > -1){
